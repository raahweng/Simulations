...
def boundary(x):
    ...
    #u[:,1] += 75
    u[:,0] += 10
    d[int(3*ny/7):int(4*ny/7), 1] += 0.02
...
def advection(un,vn,dn):
    ...
    velreverse = np.transpose(grid.reshape(nx*ny,2) - tempvel*dt*dx) # dx?
...
def animate(i):
    ...
    dye = plt.imshow(d, cmap='jet', vmax=1, interpolation='catrom')
...
plt.contourf(X, Y, p, alpha=0.5, cmap=cm.viridis)  
plt.contour(X, Y, p, cmap=cm.viridis)
dye = plt.imshow(d, cmap='hot', vmax=100, interpolation='bilinear')
plt.quiver(X[::2, ::2], Y[::2, ::2], u[::2, ::2], v[::2, ::2])
#plt.colorbar()
writer = animation.FFMpegWriter() 
ani = animation.FuncAnimation(fig, animate, interval=100, save_count = 1500)
plt.show()
