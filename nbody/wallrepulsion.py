...
m = np.random.uniform(1000000, 100000000000000000, n)
...
for i in range(traillength):
    rhist[i] = r
repulsion = 100000000000000000000
...
def step(r, v, n):
    ...
    for i in range(n):
        ...
        F[i] = Ftemp - (grav(r[i], np.array([r[i][0],400]), m[i], repulsion) + grav(r[i], np.array([r[i][0],-400]), m[i], repulsion) + grav(r[i], np.array([400,r[i][1]]), m[i], repulsion) + grav(r[i], np.array([-400,r[i][1]]), m[i], repulsion))
